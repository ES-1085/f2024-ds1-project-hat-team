...
```{r 18-19-data-joining}
data_2018_19 <- rbind(fall_2018_19_data, winter_2018_19_data, spring_2018_19_data)

write.csv(data_2018_19, "../data/final_data.csv")
```

```{r 20-21-fall-data-cleanup}
fall_2020_21_data <- read_excel("../data/2018-19 promise data.xlsx", 
    sheet = "HS RAW ALL", range = "A3:I81") |>
  janitor::clean_names() |>
  rename("category" = "x1")
...
# Create the plot
ggplot(social_emotional_data, aes(x = season, y = percent_meeting_exceeding, group = category, color = category)) +
  stat_summary(fun = "mean", geom = "line", linewidth = 1.2) +
  stat_summary(fun = "mean", geom = "point", linewidth = 3) +
  labs(title = "Trend of Percent Meeting/Exceeding Expectations", 
...
# Save the plot
ggsave("social_emotional_trend_2018_19.jpeg", path = "../data")
```
...
# Create the plot with individual observations
ggplot(social_emotional_data, aes(x = season, y = percent_meeting_exceeding)) +
  geom_jitter(width = 0.2, size = 2, alpha = 0.7, color = "#66c2a5") +  # Jittered points
  labs(title = "Individual Observations: Percent Meeting/Exceeding Expectations", 
       subtitle = "Social-Emotional Category Across 2018-19 Seasons", 
       x = "Season", 
       y = "Percent Meeting/Exceeding") +
  theme_minimal() + 
...
# Save the plot
ggsave("social_emotional_individuals_2018_19.jpeg", path = "../data")
```
